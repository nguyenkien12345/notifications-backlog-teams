...
class BacklogService:
    ...
    async def fetch_notifications(
        self, 
        min_id: Optional[int] = None, # Chỉ lấy những thông báo có ID lớn hơn số min_id này (nhằm tránh lấy lại thông báo cũ)
        ...
        logger.debug(f"Fetching notifications from Backlog: {url} with params count={count}, minId={min_id}")
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                if response.status_code == 401:
                    ...
                    logger.error(f"Space not found: Please verify your space ID ({settings.BACKLOG_SPACE_ID}) and domain ({settings.BACKLOG_DOMAIN}).")
                    response.raise_for_status()
                else:
                    response.raise_for_status()
                ...
                logger.error(f"HTTP error occurred while contacting Backlog API: {hse.response.status_code} - {hse.response.text}")
                raise hse
